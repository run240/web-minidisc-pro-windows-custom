...
BOOL LIBWDI_API wdi_is_driver_supported(int driver_type, VS_FIXEDFILEINFO* driver_info)
{
	...
	case WDI_WINUSB:
		/*
		 * Windows 7 and later ship WinUSB in-box.  This application targets
		 * those systems and uses an INF that references the system driver,
		 * so no legacy co-installer payload is required.
		 */
		return TRUE;
	case WDI_LIBUSB0:
								  ...
								  const char* inf, struct wdi_options_prepare_driver* options)
{
	...
	if (!wdi_is_driver_supported(driver_type,
		driver_type == WDI_WINUSB ? NULL : &driver_version[driver_type])) {
